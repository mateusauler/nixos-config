...
let
  ...
  mkView =
    isVpn:
    let
      netTypeName = getNetTypeName isVpn;
      inherit (cfg) baseDomain;
    in
    ''
      view "${netTypeName}" {
        match-clients { ${nets.${netTypeName}}; };
        recursion yes;
...
in
{
  ...
  config = {
    ...
    modules.dns.records.${hostName} = {
      type = "A";
      address = "192.168.0.16";
      vpnAddress = "100.69.71.75";
    };

    services.bind = {
      enable = true;
      forwarders = [
        "127.0.0.1 port ${toString piholePort}"
      ];

      cacheNetworks = lib.flatten [
        "127.0.0.0/8"
        "::1/128"
        (builtins.attrValues nets)
      ];
